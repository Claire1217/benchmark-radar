...
`AI for Science` (`ai-for-science`) appears under Agent research and includes science questions, scientific chart interpretation, prediction tasks and agent-driven scientific investigations. `Scientific Agents` (`scientific-agents`) is the narrower agent-workflow category shown immediately below it. These are distinct scopes and must not share the same display name. The `ai-for-science` URL keeps its broad scope and does not redirect to `scientific-agents` after reload.
