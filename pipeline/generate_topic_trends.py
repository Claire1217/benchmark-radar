 ...
 for t in library['manifest']['topicTaxonomy']['directions']:
  ...
  for months in (1,3,6):
   ...
   for r in rr:
    ...
    longweeks=[sum(n for d,n in r['days'] if end-timedelta(days=(29-i)*7)<=d<end-timedelta(days=(28-i)*7)) for i in range(29)]
    stars.append({k:r[k] for k in ('url','name','names')}|{'stars':n,'baseline':baseline,'previous':priorstars,'weekly':weekly,'weekly26':longweeks[3:],'average26':[sum(longweeks[i:i+4])/4 for i in range(26)],'newRepository':r['firstDay']>=start})
