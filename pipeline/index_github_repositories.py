...
import pathlib,json,re,html,urllib.request,urllib.error,urllib.parse,ipaddress,socket,concurrent.futures,hashlib,datetime,threading,os
...
def matches(name,url):
 n=norm(name);slug=norm(url.split('/')[-1]);return len(slug)>=5 and len(n)>=5 and (n in slug or slug in n)
def one(r):
 cand={};checks=[]
 def add(u,source,basis,verified=False):
  ...
  if verified:item['scope']='hosting_repo' if re.search(r'github.com/[^/]+/[^/]+/(tree|blob)/',u) else ('benchmark_repo' if matches(r['name'],canonical) else 'project_repo')
 ...
 for key in ['project','report','data']:
  ...
  if repo(u):add(u,u,'record-resource-link',key=='project' or matches(r['name'],u));continue
 ...
 for u,role in dict.fromkeys(urls):
  ...
  for url in set(re.findall(r'https?://github\.com/[A-Za-z0-9_.-]+/[A-Za-z0-9_./-]+',body)):
   ...
   add(url,u,'linked-from-'+role,matches(r['name'],url) and (role in ['project','data'] or bool(arxiv)))
